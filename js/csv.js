...
const CSVLoader = (function() {
  ...
  // Helper function to get base path (works on GitHub Pages)
  ...
  async function detectVocabularyFiles(csvFileSelect, csvMessage) {
    try {
      // Primary method: Load manifest.json (auto-generated by script)
      // The manifest.json is automatically generated by running: node generate-manifest.js
      // Use path that works both locally and on GitHub Pages
      const basePath = getBasePath();
      const manifestUrl = `${basePath}vocabulary/manifest.json`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (manifest.files && Array.isArray(manifest.files)) {
            ...
            console.log('CSV files from manifest:', csvFiles);
            
            if (csvFiles.length > 0) {
              // Verify files actually exist
              const existingFiles = await verifyFilesExist(csvFiles);
              console.log('Existing CSV files:', existingFiles);
              if (existingFiles.length > 0) {
                ...
                console.warn('No existing files found, even though manifest listed:', csvFiles);
              }
            }
          }
        } else {
        ...
        // Manifest not found - try fallback method
        console.log('Manifest file not found or error:', manifestError);
        console.log('Trying fallback detection...');
      }
      
      // Fallback method: Try to detect from config.js (if manifest doesn't exist)
      if (typeof AppConfig !== 'undefined' && AppConfig.csvFiles && AppConfig.csvFiles.length > 0) {
        ...
        if (existingFiles.length > 0) {
          populateCSVDropdown(csvFileSelect, existingFiles);
          csvMessage.textContent = `${existingFiles.length} fichier(s) CSV trouvé(s) (détection automatique). Pour une détection automatique complète, exécutez: node generate-manifest.js`;
          csvMessage.style.color = "#4caf50";
          return;
        }
      }
      ...
      csvMessage.textContent = "Aucun fichier CSV trouvé. Ajoutez des fichiers CSV dans le dossier 'vocabulary' et exécutez: node generate-manifest.js";
  ...
  async function verifyFilesExist(filenames) {
    ...
    const checkPromises = filenames.map(async (filename) => {
      try {
        // Use path that works both locally and on GitHub Pages
        const testUrl = `${basePath}vocabulary/${filename}`;
        console.log('Checking if file exists:', testUrl);
        const testResponse = await fetch(testUrl, { method: 'HEAD' });
        if (testResponse.ok) {
          console.log('File exists:', testUrl);
          return filename;
        } else {
          console.warn('File not found (status', testResponse.status, '):', testUrl);
        }
      } catch (e) {
        // File doesn't exist, return null
        console.warn('Error checking file:', testUrl, e);
        return null;
      }
      return null;
    });
